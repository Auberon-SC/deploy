...
if not STRING_SESSION1:
    LOGGER(__name__).error("STING LO MANA!")
    sys.exit()

if not API_ID:
    LOGGER(__name__).error("API_ID LO MANA!")
    sys.exit()

if not API_HASH:
    LOGGER(__name__).error("API_HASH LO MANA")
    sys.exit()
...
bot1 = (
    Client(
        ...
        api_id=API_ID,
        api_hash=API_HASH,
        plugins=dict(root="zeinzo/plugins"),
    )
    if STRING_SESSION1
    else None
)
